...
public class URWellCluster implements Comparable<URWellCluster> {    
    ...
    public void setStripOrder(int order){
        stripOrder = order;
    }
    
    public int stripOrder(){
        return stripOrder;
    }
    
    public boolean isMainStripNormal(){
        for(int accptedOrder : Constants.NORMALHITORDERS){
            if(stripOrder == accptedOrder) return true;
        }
        return false;
    }
