 ...
 * @author Tongtong Cao
 */
public class EP2EPIpPImX extends BaseAnalysis{
    ...
    public void createHistoGroupMap(){                        
        // eh+/-
        HistoGroup histoGroup = new HistoGroup("Mx(ep#rarrow e'#pi+#pi-X)", 1, 1);
        H1F hi_mx = new H1F("Mx(ep#rarrow e'#pi+#pi-X)", "", 100, 0.7, 1.4);     
        hi_mx.setTitleX("Mx(ep#rarrow e'#pi+#pi-X) (GeV)");
    ...
    public void processEvent(EventValidTracks localEvent){
        ...
        if(localEvent.getTriggerTrk() != null){
            ...
            if(localEvent.getPiPlusTrks().size() >= 1 && localEvent.getPiMinusTrks().size()>=1){
                Track piPlusTrk = localEvent.getPiPlusTrks().get(0);
                Track piMinusTrk = localEvent.getPiMinusTrks().get(0);
                ...
                X.combine(trigger, -1);
                X.combine(piPlusTrk.particle(), -1);
                X.combine(piMinusTrk.particle(), -1);
                
                histoGroupMap.get("Mx(ep#rarrow e'#pi+#pi-X)").getH1F("Mx(ep#rarrow e'#pi+#pi-X)").fill(X.mass()); 
            }
        }
    }
    
    public void postEventProcess(){        
        HistoGroup histoGroup = histoGroupMap.get("Mx(ep#rarrow e'#pi+#pi-X)");
        H1F hi_mx =  histoGroup.getH1F("Mx(ep#rarrow e'#pi+#pi-X)");
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("fit_results_ep2epi+pi-X.txt"))) {
    ...
    public static void main(String[] args){
        ...
        EP2EPIpPImX analysis = new EP2EPIpPImX();
