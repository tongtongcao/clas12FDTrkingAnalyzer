...
/**
 * Extract xy of 2 uRWell crosses and average wires from valid TB tracks, randomly abandon 1 DC cluster and 1 uRWell cross, and label such combos as 1
...
public class RealFakeCombo1URCrs5DCClsRealFake{              
    ...
    public static void main(String[] args) throws IOException {
        ...
        String outputName = "1URCrs5DCClsCombo.csv";
                ...
                while(reader.hasNext()){
                    ...
                    for(Track trk : localEvent.getTracksTB()){  
                        if(trk.isValid(true) && trk.getNumClusters() == 6 && trk.getURWellCrosses().size() == 2){                            
                            ...
                            int removedSL = (int) (Math.random() * 6) + 1;  
                            
                            // Real combos
                            for(Cluster cls : clustersInTrack){
                                if(cls.superlayer() != removedSL){
                            ...
                            for(Cluster cls : clustersNotInTrack){
                                if(cls.superlayer() != removedSL){
                                    ...
                                    for(Cluster clsInFakeTrack : clustersInFakeTrack){   
                                        if(clsInFakeTrack.superlayer() != removedSL){
                            ...
                            URWellCross fakeURCrs1 = writeClosestURWellFakeCombo(clustersInTrack, urCrossesInTrack, urCrossesNotInTrack, writer, rand, urCrossIndex, removedSL);
                            if(fakeURCrs1 != null) {
                            ...
                            URWellCross fakeURCrs2 = writeRandomURWellFakeCombo(clustersInTrack, urCrossesInTrack, urCrossesNotInTrack, writer, rand, urCrossIndex, removedSL);
                            if(fakeURCrs2 != null){
    ...
    static private URWellCross writeClosestURWellFakeCombo(List<Cluster> clustersInTrack, List<URWellCross> urCrossesInTrack, List<URWellCross> urCrossesNotInTrack, FileWriter writer, Random rand, int urCrossIndex, int removedSL) throws IOException {
    ...
    static private URWellCross writeRandomURWellFakeCombo(List<Cluster> clustersInTrack, List<URWellCross> urCrossesInTrack, List<URWellCross> urCrossesNotInTrack, FileWriter writer, Random rand, int urCrossIndex, int removedSL) throws IOException{
        URWellCross fakeURCrs = null;
        if (!urCrossesNotInTrack.isEmpty()) {
