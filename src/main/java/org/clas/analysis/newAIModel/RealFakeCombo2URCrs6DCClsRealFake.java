...
/**
 * Extract xy of 2 uRWell crosses and average wires and slopes of 6 DC clusters
 * from valid TB tracks, and label such combos as 1 Then, for a valid TB tracks,
 * use a random uRWell cross or a random cluster in the same sector to replace
 * the corresponding uRWell cross or cluster at the same region or superlayer,
 * label such combos as 0 Save them into a csv file The csv file will be input
 * as training AI model of cluster-combo classification
 *
 * @author Tongtong
 */
public class RealFakeCombo2URCrs6DCClsRealFake {
    ...
    public static void main(String[] args) throws IOException {
        ...
        String outputName = "2URCrs6DCClsCombo.csv";
                    ...
                    for (Track trk : localEvent.getTracksTB()) {
                        if (trk.isValid(true) && trk.getNumClusters() == 6 && trk.getURWellCrosses().size() == 2) {
                            ...
                            for (Cluster cls : clustersInSector) {
                                ...
                                if (clustersInTrack.size() == clsIdsInTrack.size()) {
                                    break;
                                }
                            }
                            ...
                            for (URWellCross urCrs : urCrossesInSector) {
                                ...
                                if (urCrossesInTrack.size() == urCrossesIdsInTrack.size()) {
                                    break;
                                }
                            }
                            ...
                            URWellCross fakeURCrs1 = writeClosestURWellFakeCombo(clustersInTrack, urCrossesInTrack, urCrossesNotInTrack, writer, rand);
                            if(fakeURCrs1 != null) {
                            ...
                            URWellCross fakeURCrs2 = writeRandomURWellFakeCombo(clustersInTrack, urCrossesInTrack, urCrossesNotInTrack, writer, rand);
                            if(fakeURCrs2 != null){
    ...
    static private URWellCross writeClosestURWellFakeCombo(List<Cluster> clustersInTrack, List<URWellCross> urCrossesInTrack, List<URWellCross> urCrossesNotInTrack, FileWriter writer, Random rand) throws IOException{
    ...
    static private URWellCross writeRandomURWellFakeCombo(List<Cluster> clustersInTrack, List<URWellCross> urCrossesInTrack, List<URWellCross> urCrossesNotInTrack, FileWriter writer, Random rand) throws IOException{
        URWellCross fakeURCrs = null;
        if (!urCrossesNotInTrack.isEmpty()) {
            for (Cluster cls : clustersInTrack) {
                String avgWire = String.format("%.4f", cls.avgWire());
